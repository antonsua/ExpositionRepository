...
const Vector2D Vector2D::_DEFAULT_ZERO_ANGLE_ = Vector2D(1, 0); //0 degrees
const Vector2D Vector2D::_NULL_ = Vector2D(0, 0);
const Vector2D Vector2D::_UNIT_ = Vector2D(1, 1);
...
void Vector2D::rotate(double degrees) {
	...
	//now adittion is anti-clockwise
...
{
	return angle(_DEFAULT_ZERO_ANGLE_);
}
